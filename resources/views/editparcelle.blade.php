          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ route('updateparcelle') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" value="{{ $parcelle->id}}" name="id" />
                  <div class="card-body">
              
                    <div class="form-group">
                      <label for="exampleInputEmail1">Numero</label>
                      <input type="text" class="form-control" value="{{ $parcelle->numero }}" name="numero" placeholder="Numero">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="proprietaire_id"  class="custom-select form-control-border">
                          ...
                          <option value="{{ $proprietaire->id  }}"
                            @if($parcelle->proprietaire_id == $proprietaire->id)
                                @selected(true)
                            @endif
                            >{{ $proprietaire->numero_identite }}</option>
                          @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">Date delimitation</label>
                        <input type="date" value="{{ $parcelle->date_delimation }}" class="form-control" name="Date_delimitation" placeholder="Date delimitation">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="village_id"  class="custom-select form-control-border">
                          ...
                          <option value="{{ $village->id  }}"
                            @if($parcelle->village_id == $village->id)
                                @selected(true)
                            @endif
                            >{{ $village->nom }}</option>
                          @endforeach
                  ...
                  <div class="form-group">
                    <textarea name="signature" placeholder="Signature">{{ $parcelle->signature }}</textarea>
                </div>
